...
class BaseTestStrategy(Strategy[FileSystemStorage]):
    ...
    def __init__(self, remote_storage):
        ...
        self.on_skip_hook = mock.MagicMock()
...
def test_calls_on_skip_hook_collectstatic(case: TestCase) -> None:
    clean_static_dir()
    path = create_static_file()
    cmd = Command()
    cmd.run_from_argv(["manage.py", "collectstatic", "--noinput"])
    cmd.run_from_argv(["manage.py", "collectstatic", "--noinput"])
    cmd.strategy.on_skip_hook.assert_called_once_with(
        path.name,
        path.name,
        mock.ANY,
    )
...
def test_calls_on_skip_hook_collectfast(case: TestCase) -> None:
    clean_static_dir()
    path = create_static_file()
    cmd = Command()
    cmd.run_from_argv(["manage.py", "collectstatic", "--noinput"])
...
@make_test_all_backends
@live_test
def test_call_on_skip_hook_all_backends(case: TestCase) -> None:
    clean_static_dir()
    path = create_static_file()
    cmd = Command()
    cmd.run_from_argv(["manage.py", "collectstatic", "--noinput"])
